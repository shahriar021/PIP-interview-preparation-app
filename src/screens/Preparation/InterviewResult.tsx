...
import { useSubmitINTMarksnPointsMutation } from 'src/redux/features/interview/interviewApi';
...
const InterviewResult = () => {
    ...
    useLayoutEffect(() => {
     ...
     const [submitMarksnPoints] = useSubmitINTMarksnPointsMutation()
    ...
    return (
        <View className='items-center bg-white justify-between flex-1'>
            ...
            <View className='w-full flex-col mb-4'>
                <TouchableOpacity className='border border-[#1C75AD] p-3 rounded-full m-3 items-center' onPress={()=>navigation.navigate("Interview QnA",{RID:rId})}>
                    <Text className='text-[#1C75AD] font-robotoRegular text-lg'>View All Question</Text>
                </TouchableOpacity>
                 <TouchableOpacity className='border border-[#1C75AD] p-3 rounded-full m-3 items-center flex-row justify-center gap-2' onPress={()=>navigation.navigate("BottomScreen",{screen:"Home"})}>
                    <Ionicons name="arrow-back-sharp" size={24} color="#1C75AD" />
