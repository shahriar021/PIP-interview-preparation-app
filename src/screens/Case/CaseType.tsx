    ...
    return (
        <View className='flex-1 bg-white p-3'>
            ...
            <View className='flex-col gap-3 mt-4 mb-2'>
                {getCaseTypes?.data?.map((item:string) => <TouchableOpacity key={item.id} style={{ backgroundColor: 'rgba(0,0,0,0.05)' }} className='p-4 rounded-xl border border-gray-300' onPress={()=>navigation.navigate("Case Details create",{id:item.id})}>
